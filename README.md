...
P4 当前执行动态滚动重排：它读取 P3 的无冲突基准计划，在 `area_block`、`amr_delay`、`new_task` 事件到来后冻结已完成任务，对未来任务做局部等待修复、新任务滚动插入、候选路径重评估和轨迹冲突修复。AMR 延误窗口若覆盖正在执行但尚未完成的目标 AMR 任务，P4 会释放该任务及其后续尾段并从延误结束后重新安排。

基于最新 P3 输出重新计算后，P4 滚动时域主结果满足三类硬约束：剩余轨迹冲突 0、封锁区违规 0、AMR 延误违规 0；当前总延期为 41.79，Cmax 为 73.33，F2 为 3710.20。为避免新 P3 下慢基线拖慢日常刷新，默认脚本优先生成滚动时域主结果和可视化，完整基线对比可作为单独实验运行。
