...
表 \ref{tab:p1-methods} 说明 P1 并不是简单比较几种最短路算法，而是为后续调度层生成不同偏好的基础路径源。A* 偏重栅格可达性，VG 偏重连续空间中的几何最短，AVG 引入转弯平滑性，DAVG 引入动态风险代价。后续 P2 可以在这些单算法结果之上进一步融合、去重并构造 mixed 多候选路径库。
...
这种分层也解释了为什么 P1 需要保留多种路径算法。若 P1 只输出单一最短路结果，P2 的路径选择变量就会退化；当 P1 提供多种路径偏好的单算法结果后，P2 才能进一步构建 mixed 多候选路径库，并在“短距离、少转弯、低风险、低冲突潜力”之间进行组合优化。P3/P4 则进一步使用轨迹采样表验证这些任务级路径是否在时间维度上真正可执行。
