...
```text
滚动时域重排   冲突消解成功率 100%，总延期 23.41，Cmax 61.92，F2 2182.07
...
当前搜索到的更优滚动解把新增任务 `T13` 插入 AMR4，并排在 `T11` 前执行。这样会额外扰动 `T11`，但避免把 `T13` 继续压在 AMR2 的延误尾段上，最终相对上一轮 AMR2 插入解将总延期从 27.11 降到 23.41，Cmax 从 73.24 降到 61.92，F2 从 2596.27 降到 2182.07。相对最初的 AMR2 前插解，总延期从 41.79 降到 23.41。
...
4. new_task 会把新增任务插入未来计划，并对候选插入位置做冲突感知短名单评价；
...
5. 新任务短名单修复：
   对新增任务的候选 AMR / 位置先生成轨迹并统计潜在冲突，
   同时保留每台 AMR 的代表候选和队首插入候选；
   再对短名单做有界冲突修复，并按修复后的字典序目标选择方案。
